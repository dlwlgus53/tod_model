    ...
    def __init__(self, model_name, tokenizer, data_path_prefix, seed = 1, pseudo_data = None, shuffle_mode='shuffle_session_level', 
        ...
        if data_mode == 'train':
            ...
            if self.train_data_ratio < 1.0:
                ...
                split_save_path=f'{data_path_prefix}_{self.train_data_ratio}_{self.seed}.json'
                print(f"Save the few shot data in {split_save_path}")
                with open(split_save_path, 'w') as f: json.dump(train_raw_data, f, ensure_ascii=False, indent=4)
        ...
        for idx in range(data_num):
            ...
            for turn in raw_data_list[idx]:
                ...
                for key in turn:
                    ...
                    'belief','curr_belief','prev_belief', 'confidence','gtbs',  'predbs','pred_belief_dict','pseudo']:
